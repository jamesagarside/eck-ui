...
export function MapsListPage() {
  ...
  const { data, isLoading, error } = useResourceList<ElasticMapsServer>('maps');
  ...
  return (
    <>
      <EuiPageHeader pageTitle="Elastic Maps Server" rightSideItems={[
        <EuiButton key="create" fill iconType="plusInCircle" onClick={() => navigate('/maps/create')}>Create Maps Server</EuiButton>,
      ]} />
      <EuiSpacer size="l" />
      {error && <><EuiHealth color="danger">Failed: {error.message}</EuiHealth><EuiSpacer size="m" /></>}
      <EuiBasicTable items={items} columns={columns}
        rowProps={(item: ElasticMapsServer) => ({ onClick: () => navigate(`/maps/${item.metadata.namespace}/${item.metadata.name}`), style: { cursor: 'pointer' } })}
