...
import type { ElasticMapsServer, HealthStatus } from '../../types/resources';
...
export function MapsListPage() {
  ...
  const { isConnected } = useResourceWatch('maps');

  const { data, isLoading, error, refetch } = useResourceList<ElasticMapsServer>('maps', {
    page: page + 1,
    pageSize,
  ...
  const onTableChange = ({ page: tablePage, sort }: CriteriaWithPagination<ElasticMapsServer>) => {
    if (tablePage) {
    ...
    <>
      <EuiPageHeader
        ...
        rightSideItems={canManage ? [
          <EuiButton key="create" fill iconType="plusInCircle" onClick={() => navigate('/maps/create')}>
            Create Maps Server
          </EuiButton>,
        ] : []}
      />
      ...
      {showEmptyState ? (
        <ResourceEmptyState
          resourceType="maps"
          resourceLabel="Maps Server"
          onCreate={() => navigate('/maps/create')}
        ...
        <EuiBasicTable
          ...
          sorting={{
            sort: {
              field: `metadata.${sortField}` as keyof ElasticMapsServer,
              direction: sortDirection,
            },
          }}
          onChange={onTableChange}
          rowProps={(item: ElasticMapsServer) => ({
            onClick: () => navigate(`/maps/${item.metadata.namespace}/${item.metadata.name}`),
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/maps/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
          noItemsMessage="No Elastic Maps Server instances found"
        />
      )}
    </>
  );
