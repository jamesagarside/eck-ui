...
function validateMapsForm(values: MapsFormValues): Partial<Record<keyof MapsFormValues, string>> {
  const errors: Partial<Record<keyof MapsFormValues, string>> = {};
...
export function MapsCreatePage() {
  ...
  const form = useResourceForm<MapsFormValues>({
    initialValues: {
      name: '',
      namespace: 'default',
    ...
    onSubmit: async (values) => {
      try {
        ...
        addToast({ title: `Elastic Maps '${values.name}' created successfully`, color: 'success' });
        navigate('/maps');
      } catch (err) {
        addToast({ title: 'Failed to create Elastic Maps', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
      }
    },
  });
    ...
    <>
      ...
      <EuiForm component="form" onSubmit={form.handleSubmit}>
        ...
        <EuiFlexGroup justifyContent="flexEnd">
          ...
          <EuiFlexItem grow={false}><EuiButton type="submit" fill isLoading={form.isSubmitting}>Create Maps Server</EuiButton></EuiFlexItem>
        </EuiFlexGroup>
      </EuiForm>
    </>
  );
