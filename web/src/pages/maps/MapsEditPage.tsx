...
function buildPatch(original: ElasticMapsServer, data: FormData): ElasticMapsServer {
  return {
    ...original,
    spec: {
      ...original.spec,
...
export function MapsEditPage() {
  ...
  useEffect(() => {
    if (mapsData && !initialized) {
      const mapsServer = mapsData as ElasticMapsServer;
      const newFormData: FormData = {
        version: mapsServer.spec.version,
        count: mapsServer.spec.count,
        elasticsearchRef: mapsServer.spec.elasticsearchRef || null,
      };
      setFormData(newFormData);

      // Set selected ES cluster
      if (mapsServer.spec.elasticsearchRef) {
        const esRef = mapsServer.spec.elasticsearchRef;
  ...
  }, [mapsData, initialized, namespace]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const patch = buildPatch(mapsData as ElasticMapsServer, formData);
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Failed to update Elastic Maps Server');
    }
  };
  ...
  const previewYaml = buildPatch(mapsData as ElasticMapsServer, formData);
  const originalYaml = mapsData as ElasticMapsServer;
        ...
        <EuiForm component="form" onSubmit={handleSubmit}>
          ...
          <EuiPanel>
            ...
            <EuiFlexGroup>
              ...
              <EuiFlexItem>
                <EuiFormRow
                  label="Count"
                  helpText="Number of Elastic Maps Server pods"
          ...
          <EuiPanel>
            ...
            <EuiFormRow
              ...
              helpText="Elastic Maps Server requires an Elasticsearch cluster"
