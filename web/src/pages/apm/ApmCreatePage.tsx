...
function validateApmForm(values: ApmFormValues): Partial<Record<keyof ApmFormValues, string>> {
  const errors: Partial<Record<keyof ApmFormValues, string>> = {};
...
export function ApmCreatePage() {
  ...
  const form = useResourceForm<ApmFormValues>({
    initialValues: {
      name: '',
      namespace: 'default',
    ...
    onSubmit: async (values) => {
      const resource = {
        ...
        spec: {
          ...
          ...(values.kibanaRef ? { kibanaRef: { name: values.kibanaRef } } : {}),
        },
      };
      try {
        await createMutation.mutateAsync(resource);
        addToast({ title: `APM Server '${values.name}' created successfully`, color: 'success' });
        navigate('/apm');
      } catch (err) {
        addToast({ title: 'Failed to create APM Server', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
      }
    },
  });
    ...
    <>
      ...
      <EuiForm component="form" onSubmit={form.handleSubmit}>
        ...
        <EuiFlexGroup justifyContent="flexEnd">
          ...
          <EuiFlexItem grow={false}>
            <EuiButton type="submit" fill isLoading={form.isSubmitting}>Create APM Server</EuiButton>
          </EuiFlexItem>
        </EuiFlexGroup>
      </EuiForm>
    </>
