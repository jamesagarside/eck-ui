...
function EditForm({ resource, namespace, name }: EditFormProps) {
  ...
  const form = useResourceForm<ApmEditFormValues>({
    initialValues: {
    ...
    onSubmit: async (values) => {
      try {
        ...
        addToast({ title: `APM Server '${name}' updated`, color: 'success' });
        navigate(`/apm/${namespace}/${name}`);
      } catch (err) {
        addToast({ title: 'Failed to update APM Server', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
      }
    },
  });
      ...
      <EuiForm component="form" onSubmit={form.handleSubmit}>
        <EuiPanel>
          ...
          <EuiFormRow label="Kibana Reference (optional)">
            <EuiFieldText value={form.fields.kibanaRef.value as string} onChange={(e) => form.fields.kibanaRef.onChange(e.target.value)} placeholder="my-kibana" />
          </EuiFormRow>
        </EuiPanel>
        <EuiSpacer size="l" />
