...
export function ApmDetailPage() {
  ...
  const podsQuery = usePods(namespace || '', buildECKLabelSelector('apm', name || ''));
  const eventsQuery = useEvents(namespace || '');

  if (isLoading) return <DetailSkeleton />;
  if (error || !resource) return <EuiCallOut title="Failed to load APM Server" color="danger" iconType="error">{error?.message || 'Not found'}</EuiCallOut>;

  const health = resource.status?.health || 'unknown';
  const phase = resource.status?.phase || 'Unknown';

  const handleDelete = async () => { if (namespace && name) { await deleteMutation.mutateAsync({ namespace, name }); navigate('/apm'); } };
  ...
  const overviewItems = [
    ...
    { title: 'Kibana Ref', description: resource.spec.kibanaRef?.name || '-' },
  ...
  return (
    <>
      <EuiPageHeader pageTitle={resource.metadata.name} iconType="logoAPM" description={`Namespace: ${resource.metadata.namespace}`}
        rightSideItems={[
          <EuiButton key="edit" onClick={() => navigate(`/apm/${namespace}/${name}/edit`)}>Edit</EuiButton>,
