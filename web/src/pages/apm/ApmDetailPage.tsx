...
export function ApmDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast({ title: `APM Server '${name}' deleted`, color: 'success' });
      navigate('/apm');
    } catch (err) {
      addToast({ title: 'Failed to delete APM Server', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
    }
  };
