...
export function ApmListPage() {
  ...
  const { isConnected } = useResourceWatch('apm');

  const { data, isLoading, error, refetch } = useResourceList<ApmServer>('apm', {
    page: page + 1,
    pageSize,
  ...
  const onTableChange = ({ page: tablePage, sort }: CriteriaWithPagination<ApmServer>) => {
    if (tablePage) {
    ...
    <>
      ...
      {showEmptyState ? (
        ...
        <EuiBasicTable
          ...
          sorting={{
            sort: {
              field: `metadata.${sortField}` as keyof ApmServer,
              direction: sortDirection,
            },
          }}
          onChange={onTableChange}
          rowProps={(item: ApmServer) => ({
            onClick: () => navigate(`/apm/${item.metadata.namespace}/${item.metadata.name}`),
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/apm/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
