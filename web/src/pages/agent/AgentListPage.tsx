...
function getDisplayMode(agent: Agent): string {
  if (agent.spec.fleetServerRef) return 'Fleet-connected';
  return 'Standalone';
}
...
export function AgentListPage() {
  ...
  const queryParams = new URLSearchParams({ mode: 'standalone' });
  ...
  const { data, isLoading, error, refetch } = useQuery<ResourceList<Agent>>({
    queryKey: ['resources', 'agent', { mode: 'standalone', page: page + 1, pageSize, search, health: healthParam, sort: sortField, order: sortDirection }],
  ...
  const columns: EuiBasicTableColumn<Agent>[] = [
    ...
    { field: 'spec.version', name: 'Version', width: '100px' },
    {
      ...
      width: '140px',
      render: (agent: Agent) => {
        const mode = getDisplayMode(agent);
        return <EuiBadge color={mode === 'Standalone' ? 'hollow' : 'primary'}>{mode}</EuiBadge>;
      },
    },
    {
      field: 'status.health',
    ...
    <>
      ...
      {showEmptyState ? (
        ...
        <EuiBasicTable
          ...
          rowProps={(item: Agent) => ({
            onClick: () => navigate(`/agent/${item.metadata.namespace}/${item.metadata.name}`),
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/agent/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
