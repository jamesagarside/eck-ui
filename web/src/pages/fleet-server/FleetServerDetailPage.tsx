...
export function FleetServerDetailPage() {
  ...
  const podsQuery = usePods(namespace || '', buildECKLabelSelector('agent', name || ''));
  ...
  const esRefName = resource.spec.elasticsearchRefs?.[0]?.name;
  const handleDelete = async () => {
    if (!namespace || !name) return;
    try {
      await deleteMutation.mutateAsync({ namespace, name });
      addToast({ title: `Fleet Server '${name}' deleted`, color: 'success' });
      navigate('/fleet-server');
    } catch (err) {
      addToast({ title: 'Failed to delete Fleet Server', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
    }
  };
  ...
  const overviewItems = [
    ...
    { title: 'Elasticsearch Ref', description: esRefName || '-' },
  ];
  ...
  return (
    <>
      <EuiPageHeader pageTitle={resource.metadata.name} iconType="fleetApp" description={`Namespace: ${resource.metadata.namespace}`}
        rightSideItems={canManage ? [
          <EuiButton key="edit" onClick={() => navigate(`/fleet-server/${namespace}/${name}/edit`)} aria-label={`Edit ${resource.metadata.name}`}>Edit</EuiButton>,
