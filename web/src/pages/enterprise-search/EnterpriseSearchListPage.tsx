...
export function EnterpriseSearchListPage() {
  ...
  const { isConnected } = useResourceWatch('enterprise-search');

  const { data, isLoading, error, refetch } = useResourceList<EnterpriseSearch>('enterprise-search', {
    page: page + 1,
    pageSize,
  ...
  const onTableChange = ({ page: tablePage, sort }: CriteriaWithPagination<EnterpriseSearch>) => {
    if (tablePage) {
    ...
    <>
      <EuiPageHeader
        ...
        rightSideItems={canManage ? [
          <EuiButton key="create" fill iconType="plusInCircle" onClick={() => navigate('/enterprise-search/create')}>
      ...
      {showEmptyState ? (
        <ResourceEmptyState
          resourceType="enterprise-search"
          resourceLabel="Enterprise Search"
          onCreate={() => navigate('/enterprise-search/create')}
        ...
        <EuiBasicTable
          ...
          sorting={{
            sort: {
              field: `metadata.${sortField}` as keyof EnterpriseSearch,
              direction: sortDirection,
            },
          }}
          onChange={onTableChange}
          rowProps={(item: EnterpriseSearch) => ({
            onClick: () => navigate(`/enterprise-search/${item.metadata.namespace}/${item.metadata.name}`),
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/enterprise-search/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
