...
export function KibanaListPage() {
  ...
  const { isConnected } = useResourceWatch('kibana');

  const { data, isLoading, error, refetch } = useResourceList<Kibana>('kibana', {
    page: page + 1,
    pageSize,
  ...
  const onTableChange = ({ page: tablePage, sort }: CriteriaWithPagination<Kibana>) => {
    if (tablePage) {
    ...
    <>
      ...
      {showEmptyState ? (
        ...
        <EuiBasicTable
          ...
          sorting={{
            sort: {
              field: `metadata.${sortField}` as keyof Kibana,
          ...
          rowProps={(item: Kibana) => ({
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/kibana/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
