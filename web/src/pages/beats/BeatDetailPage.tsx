...
export function BeatDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast({ title: `Beat '${name}' deleted`, color: 'success' });
      navigate('/beats');
    } catch (err) {
      addToast({ title: 'Failed to delete Beat', color: 'danger', text: err instanceof Error ? err.message : 'An unexpected error occurred' });
    }
  };
  ...
  return (
    <>
      <EuiPageHeader pageTitle={resource.metadata.name} iconType="logoBeats" description={`Namespace: ${resource.metadata.namespace}`}
        rightSideItems={[
          <EuiButton key="edit" onClick={() => navigate(`/beats/${namespace}/${name}/edit`)} aria-label={`Edit ${resource.metadata.name}`}>Edit</EuiButton>,
