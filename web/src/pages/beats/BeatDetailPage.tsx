...
import type { Beat, HealthStatus, ResourceEvent } from '../../types/resources';
...
export function BeatDetailPage() {
  ...
  const { data: resource, isLoading, error } = useResource<Beat>('beat', namespace || '', name || '');
  const deleteMutation = useDeleteResource('beat');
  const updateMutation = useUpdateResource('beat');
  const podsQuery = usePods(namespace || '', buildECKLabelSelector('beat', name || ''));
  const eventsQuery = useEvents(namespace || '');

  if (isLoading) return <DetailSkeleton />;
  if (error || !resource) return <EuiCallOut title="Failed to load Beat" color="danger" iconType="error">{error?.message || 'Not found'}</EuiCallOut>;

  const health = resource.status?.health || 'unknown';
  const phase = resource.status?.phase || 'Unknown';
  const handleDelete = async () => { if (namespace && name) { await deleteMutation.mutateAsync({ namespace, name }); navigate('/beats'); } };

  const overviewItems = [
    { title: 'Name', description: resource.metadata.name },
    { title: 'Namespace', description: resource.metadata.namespace },
    { title: 'Type', description: resource.spec.type },
  ...
  return (
    <>
      <EuiPageHeader pageTitle={resource.metadata.name} iconType="logoBeats" description={`Namespace: ${resource.metadata.namespace}`}
        rightSideItems={[
          <EuiButton key="edit" onClick={() => navigate(`/beats/${namespace}/${name}/edit`)}>Edit</EuiButton>,
