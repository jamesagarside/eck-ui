...
export function BeatListPage() {
  ...
  const { isConnected } = useResourceWatch('beat');

  const { data, isLoading, error, refetch } = useResourceList<Beat>('beat', {
    page: page + 1,
    pageSize,
  ...
  const onTableChange = ({ page: tablePage, sort }: CriteriaWithPagination<Beat>) => {
    if (tablePage) {
    ...
    <>
      <EuiPageHeader
        ...
        rightSideItems={canManage ? [
          <EuiButton key="create" fill iconType="plusInCircle" onClick={() => navigate('/beats/create')}>
            Create Beat
          </EuiButton>,
        ] : []}
      />
      ...
      {showEmptyState ? (
        <ResourceEmptyState
          resourceType="beat"
          resourceLabel="Beat"
          onCreate={() => navigate('/beats/create')}
        ...
        <EuiBasicTable
          ...
          sorting={{
            sort: {
              field: `metadata.${sortField}` as keyof Beat,
              direction: sortDirection,
            },
          }}
          onChange={onTableChange}
          rowProps={(item: Beat) => ({
            onClick: () => navigate(`/beats/${item.metadata.namespace}/${item.metadata.name}`),
            ...
            onKeyDown: (e: React.KeyboardEvent) => {
              if (e.key === 'Enter') {
                navigate(`/beats/${item.metadata.namespace}/${item.metadata.name}`);
              }
            },
          })}
